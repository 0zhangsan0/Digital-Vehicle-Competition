...
class TrainTestSplit:
    def __init__(self, feature_path, save_path, soh_Path):
        ...
        sohFiles = [item for item in os.listdir(self.sohPath) if item.endswith('_redefineCapacity_IQR.csv')]
        ...
        def assignmentSOH(x):
            ...
            if group in self.groups:
                x['SOH'] = f2.loc[f2['group'] == group, 'SOH'].values[0]
                return x
...
class ExtendLayer1(nn.Module):
    def __init__(self):
        ...
        self.linear = nn.Linear(in_features=12, out_features=128, bias=True)
...
class ExtendLayer2(nn.Module):
    def __init__(self):
        ...
        self.linear = nn.Linear(in_features=6, out_features=64, bias=True)
        self.extend_cnn = nn.Conv2d(in_channels=1, out_channels=20, kernel_size=(5, 3), stride=(2, 1),
                                    padding=(2, 1))
        self.activation = nn.ReLU()
...
class FeedForward(nn.Module):
    def __init__(self):
        ...
        self.dense1 = nn.Linear(in_features=40, out_features=32, bias=True)
        self.relu = nn.ReLU()
        self.dense2 = nn.Linear(in_features=32, out_features=16, bias=True)
        self.dense3 = nn.Linear(in_features=40, out_features=16, bias=True)
...
if __name__ == '__main__':
    featurePath = 'E:\\pycharm\\DigitalCarRace\\CapacityFeatureFile'
    sohPath = r'E:\pycharm\DigitalCarRace\RedefineSOH'
    savePath = r'E:\pycharm\DigitalCarRace\CapacityModel1_2_4'
