...
if __name__ == '__main__':
    model2 = torch.load(r'E:\pycharm\DigitalCarRace\CapacityModel1_2_3\CapacityEstimationModel3100.pkl')
    ...
    for i in range(file_nums):
        ...
        f2 = pd.DataFrame([item for item in zip(data.SOHs, data.mileages_soh)],
                          columns=['安时容量', '累计里程_Ah'])
        f1.sort_values(['累计里程_soh'], inplace=True)
        f2.sort_values(['累计里程_Ah'], inplace=True)
        plt.plot(f1['累计里程_soh'], f1['Pred'], 'r*')
        plt.plot(f2['累计里程_Ah'], f2['安时容量'], 'k*-')
