...
class LSTMModel(nn.Module):
    def __init__(self, d_k, d_v, num_heads, d_model, encoder_hidden_size, lstm_hidden_size, num_layers, drop_out=0.1):
        ...
        self.lstm = nn.LSTM(input_size=336, hidden_size=self.lstm_hidden_size, num_layers=self.num_layers,
...
class ResNet(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv1d(in_channels=96, out_channels=32, kernel_size=3, stride=1, padding=1)
        self.conv2 = nn.Conv1d(in_channels=32, out_channels=16, kernel_size=2, stride=1, padding=0)
        self.shortcut = nn.Conv1d(in_channels=96, out_channels=16, kernel_size=2, stride=1, padding=0)
        self.conv3 = nn.Conv1d(in_channels=16, out_channels=4, kernel_size=1, stride=1, padding=0)
        self.activation = nn.ReLU()
...
class FeatureModel(nn.Module):
    ...
    def forward(self, cell_features, temperature_features):
        ...
        # (batch*seq_len,96,17),(batch*seq_len,96,8)
