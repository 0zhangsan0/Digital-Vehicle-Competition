...
class CapacityEstimation(nn.Module):
    def __init__(self):
        ...
        self.pool = nn.AvgPool1d(kernel_size=13, stride=1, padding=6)
    ...
    def forward(self, cell_features, temperature_features):
        ...
        x = self.fc(x)
        x = self.linear(x)
        x = self.pool(x.reshape(1, -1)).reshape(-1, 1)
        return x
...
if __name__ == '__main__':
    ...
    savePath = r'E:\pycharm\DigitalCarRace\CapacityModel3\pool'
    ...
    train_loader = DataLoader(trainData, batch_size=135, shuffle=True, num_workers=10, drop_last=True)
    ...
    # model2 = torch.load(r'E:\pycharm\DigitalCarRace\CapacityModel3\CapacityEstimationModel500.pkl').to(device)
