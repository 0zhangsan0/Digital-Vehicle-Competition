...
if __name__ == '__main__':
    ...
    while True:
        ...
        if epoch % 100 == 0:
            ...
            df_loss.to_csv(os.path.join(savePath, f'CapacityEstimationModel_loss2.csv'), index=False)
            loss2 += [valid()]
            pd.DataFrame(loss2, columns=['loss']).to_csv(
                os.path.join(savePath, f'CapacityEstimationModel_valid_loss2.csv'), index=False)
