...
class LSTMModel(nn.Module):
    def __init__(self, d_k, d_v, num_heads, d_model, encoder_hidden_size, lstm_hidden_size, num_layers, drop_out=0.1):
        ...
        self.linear=nn.Linear(in_features=160,out_features=40,bias=True)
...
savePath = r'E:\pycharm\DigitalCarRace\DeclineModel2'
    ...
    with torch.no_grad():
        for data in valid_loader:
            ...
            pd.DataFrame(comp, columns=['pred', 'target']).to_csv(
                f'E:/pycharm/DigitalCarRace/DeclineModel2/DeclineModel{epoch}.csv', index=False)
            return loss.item()
